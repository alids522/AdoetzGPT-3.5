<script lang="ts">
	...
	import { estimateTokens, formatTokenCount, resolveModelContextLimit } from '$lib/stores/tokenUsage';
	...
	export let taskIds: any[] | null = null;
	...
	$: activeContextModel = $models.find((model) => model.id === selectedModelIds?.[0]);
	$: contextLimit = resolveModelContextLimit(activeContextModel);
	$: contextUsedTokens =
		(history?.currentId ? createMessagesList(history, history.currentId) : []).reduce(
		...
		(files ?? []).reduce(
			(total, file) =>
				total +
				estimateTokens(
					file?.content ?? file?.text ?? file?.name ?? file?.filename ?? file?.meta?.name ?? ''
				),
			0
		);
	$: contextUsageRatio = contextLimit > 0 ? Math.min(contextUsedTokens / contextLimit, 1) : 0;
	$: contextUsageLabel = `${formatTokenCount(contextUsedTokens)} / ${formatTokenCount(contextLimit)}`;
				...
				<div class="">
					...
					>
						...
						>
							...
							<div class="mx-2 mt-1.5 flex flex-wrap items-center gap-2 text-[11px] text-gray-500 dark:text-gray-400">
								<div
									class="h-1.5 min-w-24 flex-1 overflow-hidden rounded-full bg-gray-100 dark:bg-gray-800"
									title={$i18n.t('Context window usage')}
								>
									<div
										class="h-full rounded-full transition-all {contextUsageRatio > 0.9
											? 'bg-red-500'
											: contextUsageRatio > 0.7
												? 'bg-amber-500'
												: 'bg-sky-500'}"
										style={`width: ${Math.max(2, contextUsageRatio * 100)}%;`}
									></div>
								</div>
								<div class="shrink-0 font-medium tabular-nums">
									{contextUsageLabel}
								</div>
							</div>
									...
									{:else}
										...
										{#if prompt === '' && files.length === 0 && ($_user?.role === 'admin' || ($_user?.permissions?.chat?.call ?? true))}
											<div class=" flex items-center">
												...
												<!-- Gemini Live Voice Chat Button -->
												{#if $_user?.role === 'admin'}
													<Tooltip content={$i18n.t('Gemini Live')}>
														<button
															class="bg-black text-white hover:bg-gray-900 dark:bg-white dark:text-black dark:hover:bg-gray-100 transition rounded-full p-1.5 self-center ml-1"
															type="button"
															on:click={async () => {
																if (selectedModels.length > 1) {
																	toast.error($i18n.t('Select only one model to call'));
																...
																try {
																	...
																	showGeminiLiveOverlay.set(true);
																		// Only open Controls panel on desktop, not mobile (mobile uses fixed overlay)
																		if (!$mobile) {
																			showControls.set(true);
																		}
																} catch (err) {
														...
														>
															...
															>
																...
																<path
																	stroke-linecap="round"
																	stroke-linejoin="round"
																	d="M17.25 8.25c2.25 0 4.5 1.5 4.5 4.5"
																/>
																<path
																	stroke-linecap="round"
																	stroke-linejoin="round"
																	d="M20.25 5.25c3.75 0 6.75 3 6.75 6.75"
																/>
															</svg>
														</button>
													</Tooltip>
