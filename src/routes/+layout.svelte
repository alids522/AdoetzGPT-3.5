<script>
	...
	import BackendSetup from '$lib/components/common/BackendSetup.svelte';
	...
	onMount(async () => {
		// Initialize Capacitor bridge for native WebView optimizations
		initCapacitorBridge();

		// Check if remote backend URL has been configured
...
{#if loaded}
	{#if !backendUrlSet}
		<BackendSetup onSave={handleSaveBackendUrl} />
	{:else if $isApp}
